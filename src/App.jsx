...
/* Linha do tempo de uma leva (tudo termina junto):
   total = item mais longo; cada item começa em (total - tempoItem). */
...
export default function App() {
  const [catalog, setCatalog] = useState([]);   // templates persistidos
  const [staged, setStaged] = useState([]);      // ids do catálogo no "carrinho" da próxima leva
  const [batches, setBatches] = useState([]);    // levas (running | paused | done)
  ...
  const firedRef = useRef({}); // { batchId: Set<eventId> }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (alive && s) {
        ...
        // reconstrói "fired" marcando eventos já passados (sem tocar som)
        const t = Date.now();
        bs.forEach((b) => {
          const sch = scheduleOf(b.items);
          const el = (b.status === 'running' ? b.accMs + (t - b.startTs) : b.accMs) / 1000;
  ...
  /* salvar (não roda a cada tick: batches só muda em ações/conclusão) */
  useEffect(() => {
    if (loaded) saveState({ catalog, staged, batches, seq });
  }, [catalog, staged, batches, seq, loaded]);

  /* relógio: só roda se houver leva ativa */
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      batches.forEach((b) => {
        ...
        const sch = scheduleOf(b.items);
        const el = (b.accMs + (t - b.startTs)) / 1000;
  ...
  const startLeva = () => {
    ...
    setBatches((p) => [...p, { id, label: 'Leva ' + seq, items, status: 'running', startTs: Date.now(), accMs: 0 }]);
    setSeq((s) => s + 1);
    setStaged([]);
  };
  ...
  /* resumo global no topo (próxima ação mais próxima entre as levas ativas) */
  ...
  const evLabel = (ev) =>
    ev.type === 'start' ? `Iniciar "${ev.name}"`
    : ev.type === 'split' ? `Metade de "${ev.name}"`
    : 'Tudo pronto!';
  ...
  const css = `
    ...
    .cs-spacer{ flex:1; }
    .cs-stagebtn{ border:1px solid var(--bd); background:var(--surf); color:var(--tx); border-radius:8px; padding:7px 11px; font-size:12px; cursor:pointer; font-family:'Sora'; white-space:nowrap; }
  ...
  const renderLevaItems = (b, finished) => {
    ...
    // próximo item a iniciar (menor offset ainda não começado)
    const nextStartId = (() => {
    ...
    return (
      <div className="cs-board">
        {b.items.map((it) => {
          ...
          let cls = 'wait', stat;
          if (finished || el >= sch.totalSec) { cls = 'dn'; stat = '✓ pronto'; }
          else if (el < startAt) { cls = it.id === nextStartId ? 'next' : 'wait'; stat = `inicia em ${fmt(startAt - el)}`; }
          else { cls = 'run'; stat = `restam ${fmt(rem)}`; }
          return (
    ...
    <div className="cs-wrap">
      ...
      <div className="cs-shell">
        <div className="cs-head">
          <h1 className="cs-title">Cronômetro <span>Sincronizado</span></h1>
          <button className="cs-soundbtn" onClick={() => setSoundOn((s) => !s)}>🔊 Som <b>{soundOn ? 'ligado' : 'mudo'}</b></button>
        </div>

        {/* RESUMO */}
        <div className="cs-summary">
          <span className="pill"><b>{active.length}</b> {active.length === 1 ? 'leva ativa' : 'levas ativas'}</span>
          {summary ? (
            <span className="nxt">
              Próxima ação · <b>{evLabel(summary.ev)}</b> ({summary.leva}) em <span className="cd">{fmt(summary.inSec)}</span>
            </span>
          ) : (
            <span className="nxt">Nenhuma leva em andamento.</span>
          )}
        </div>

        {/* 1. CADASTRO */}
        <div className="cs-sec">
          <div className="cs-sechead"><span className="tag" style={{ background: 'var(--amber)' }} /><h2>Cadastro de item</h2></div>
          <div className="cs-panel">
            <div className="cs-form">
              <input className="cs-inp" placeholder="Nome do item (ex: Arroz)" value={name} onKeyDown={onFormKey} onChange={(e) => setName(e.target.value)} />
              <div className="cs-row2">
                <input className="cs-inp" placeholder="min" inputMode="numeric" value={min} onKeyDown={onFormKey} onChange={(e) => setMin(e.target.value.replace(/\D/g, ''))} />
                <input className="cs-inp" placeholder="seg" inputMode="numeric" value={sec} onKeyDown={onFormKey} onChange={(e) => setSec(e.target.value.replace(/\D/g, ''))} />
              </div>
              <div className="cs-splitrow">
                <div className="t">Split (bipe na metade)<small>Toca um som ao passar metade do tempo do item</small></div>
                <div className={'cs-switch' + (split ? ' on' : '')} onClick={() => setSplit((s) => !s)} role="switch" aria-checked={split}><span className="knob" /></div>
              </div>
              <button className="cs-add" onClick={addItem} disabled={!name.trim()}>+ Adicionar ao catálogo</button>
            </div>
          </div>
        </div>
        ...
        {/* 2. LISTA / CATÁLOGO */}
        <div className="cs-sec">
          <div className="cs-sechead"><span className="tag" style={{ background: 'var(--blue)' }} /><h2>Lista de itens</h2><span className="num">{catalog.length}</span></div>
          <div className="cs-panel">
            {catalog.length === 0 ? <div className="cs-empty">Nenhum item cadastrado ainda.</div>
              : catalog.map((c) => {
                ...
                return (
                  <div className={'cs-litem' + (on ? ' staged' : '')} key={c.id}>
                    ...
                    <button className={'cs-stagebtn' + (on ? ' on' : '')} onClick={() => toggleStage(c.id)}>
                      {on ? '✓ no preparo' : '+ preparo'}
                    </button>
                    <button className="cs-del" onClick={() => removeCatalog(c.id)} title="Remover do catálogo">×</button>
                  </div>
                );
              })}
          </div>
        ...
        {/* 3. BOARD DE PREPARO (próxima leva) */}
        <div className="cs-sec">
          <div className="cs-sechead"><span className="tag" style={{ background: 'var(--amber)' }} /><h2>Board de preparo</h2><span className="num">{stagedItems.length}</span></div>
          <div className="cs-panel">
            {stagedItems.length === 0 ? <div className="cs-empty">Marque itens na lista (+ preparo) para montar a próxima leva.</div>
              : (
                <>
                  <div className="cs-board">
                    {stagedItems.map((c) => (
                      <div className="cs-card" key={c.id}>
                        ...
                        <div className="cstat">começa +{fmt(stagedSchedule.totalSec - c.totalSec)}</div>
                      </div>
                    ))}
                  </div>
                  <button className="cs-startbtn" onClick={startLeva}>▶ Iniciar leva · termina em {fmt(stagedSchedule.totalSec)}</button>
                  <button className="cs-clearbtn" onClick={() => setStaged([])}>Limpar preparo</button>
                </>
              )}
          </div>
        </div>

        {/* 4. EM EXECUÇÃO (levas paralelas) */}
        <div className="cs-sec">
          <div className="cs-sechead"><span className="tag" style={{ background: 'var(--mint)' }} /><h2>Em execução</h2><span className="num">{active.length}</span></div>
          <div className="cs-panel">
            {active.length === 0 ? <div className="cs-empty">As levas iniciadas aparecem aqui, cada uma com seu cronômetro.</div>
              : active.map((b) => {
                const sch = scheduleOf(b.items);
                const el = elapsedOf(b);
                const rem = Math.max(0, sch.totalSec - el);
                  ...
                  <div className={'cs-leva ' + b.status} key={b.id}>
                    <div className="cs-leva-top">
                      <div>
                        ...
                        <div className="cs-leva-state">{b.status === 'running' ? 'em andamento' : 'pausada'}</div>
                      </div>
                      <div className="cs-leva-time">{fmt(rem)}</div>
                    </div>
                    {nx && b.status === 'running' && (
                      <div className="cs-leva-next">Próxima: {evLabel(nx)} em <b>{fmt(nx.at - el)}</b></div>
                    )}
                    {renderLevaItems(b, false)}
                    <div className="cs-leva-ctrl" style={{ marginTop: 12 }}>
                      {b.status === 'running'
                        ? <button className="cs-mini" onClick={() => pauseLeva(b.id)}>Pausar</button>
                        : <button className="cs-mini" onClick={() => resumeLeva(b.id)}>Retomar</button>}
                      <button className="cs-mini" onClick={() => zeroLeva(b.id)}>Zerar</button>
                      <span className="cs-spacer" />
                      <button className="cs-mini danger" onClick={() => removeLeva(b.id)}>Descartar</button>
                    </div>
                  </div>
                );
              })}
        ...
        {/* 5. DONE */}
        <div className="cs-sec">
          <div className="cs-sechead"><span className="tag" style={{ background: 'var(--green)' }} /><h2>Done</h2><span className="num">{done.length}</span></div>
          <div className="cs-panel">
            {done.length === 0 ? <div className="cs-empty">Levas concluídas aparecem aqui.</div>
              : done.map((b) => (
                <div className="cs-leva dn" key={b.id}>
                  <div className="cs-leva-top">
                    <div><div className="cs-leva-label">{b.label}</div><div className="cs-leva-state">concluída</div></div>
                    <div className="cs-leva-time">✓ 00:00</div>
                  </div>
                  {renderLevaItems(b, true)}
                  <div className="cs-leva-ctrl" style={{ marginTop: 12 }}>
                    <span className="cs-spacer" />
                    <button className="cs-mini danger" onClick={() => removeLeva(b.id)}>Limpar</button>
                  </div>
                </div>
              ))}
          </div>
      ...
      {flash && (
        <div className={`cs-toast ${flash.type}`}>
          <div className="k">{flash.leva} · {flash.type === 'start' ? 'Agora' : flash.type === 'split' ? 'Metade' : 'Fim'}</div>
